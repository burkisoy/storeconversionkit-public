/*
  # Add customize count tracking
  
  1. New Tables
    - Create sections table if it doesn't exist
    - Add customize_count column
    - Add index for sorting by customize count
...
BEGIN
  UPDATE sections
  SET 
    customize_count = customize_count + 1,
    updated_at = now()
  WHERE id = section_id;
END;
$$;

-- Insert initial data for existing sections if not exists
