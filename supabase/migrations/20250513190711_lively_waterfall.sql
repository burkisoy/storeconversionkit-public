...
-- Create policy for users to read their own records if it doesn't exist
DO $$
BEGIN
  IF NOT EXISTS (
...
-- Create policy for users to update their own records if it doesn't exist
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies 
...
WHERE id IN (
  ...
  WHERE email = 'mymilliondollarcodes@gmail.com'
);